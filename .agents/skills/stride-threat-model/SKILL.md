...
## Goal
Guide the agent to analyze the workspace directory structure, configuration files,
and code files to produce a structured `threat_model.md` assessment.

## Instructions
1. **Analyze System Boundaries**: Map the entry points (tools, workflows, prompts)
...
2. **STRIDE Evaluation**: Evaluate the system against the six STRIDE pillars:
   ...
   - **Elevation of Privilege**: Can an unauthenticated user bypass access control
     to reach privileged tool actions?
3. **Output**: Generate a structured `threat_model.md` saved directly into the
   workspace root, covering CourseCraft AI's actual entry points (roadmap
   generation endpoint, nudge generation endpoint, module-completion state
   updates, localStorage-persisted progress).
